...
- Developer has already followed the user guide and can run:
...
└─ (other subfolders: data/, tmp/, etc., if used)
...
### 4.1 High-level user flow
  ...
  2.	On start:
o	CSV files are ensured.
o	Snoozes older than a few days are cleaned.
o	Tabs are created: Pillbox, Add/Edit, Summary.
  3.	User typically:
o	Goes to Add/Edit to add one or more medications.
o	Returns to Pillbox to see the weekly grid.
o	Waits for reminders/popups while the app runs.
o	Uses Summary tab to see recent actions.
...
#### 4.2.1 Helper functions (top-level)
  •	CSV utilities
o	ensure_csv(path, headers)
o	read_rows(path)
o	append_row(path, headers, row)
o	write_all(path, headers, rows)
These functions provide a thin wrapper over csv.DictReader/Writer.
  
  •	Schedule helpers
o	days_mask_to_names(mask)
Converts '1010100' style masks into day names ("Mon Wed Fri").
o	coerce_days_mask_from_bools(day_bools)
Maps list of 7 booleans to string mask.
o	is_day_active(mask, d)
...
o	log_action(med_id, sched_dt, action, actual_dt=None)
Appends a log entry to dose_log.csv.
o	is_already_logged(med_id, sched_dt)
...
Creates:
o	Title label.
o	Weekly grid labels (self.grid_labels[(bucket, weekday_index)]).
o	Color legend.
o	Action buttons (Refresh, Clear old snoozes).
...
•	_update_grid_colors()
o	Resets all cells to idle.
o	Loops over week = _build_week_schedule() and logs from dose_log.csv.
o	Applies colors according to action:
	taken → green
	skipped → red
	snoozed → blue (and shows “(snoozed)”)
	due soon today → yellow
	otherwise → gray/normal.
o	Relies on bucket_for_hour() and the BUCKETS map.
•	_clear_old_snoozes(keep_days) and _refresh_ui()
o	Utility handlers for buttons.

B. Add/Edit tab
•	_build_edit_tab()
Builds the form:
o	self.ent_name, self.ent_dose
o	Time pickers: self.cb_hour, self.cb_minute, and "Add time" → _add_time_to_list()
o	Times list: self.times_listbox and "Remove selected" → _remove_selected_time()
o	Day checkboxes: self.day_vars
o	Active checkbox: self.var_active
o	"Save Medication" → _save_medication()
o	Current Medications table: self.tree using a simplified view of schedule.
o	"Reload list", "Edit selected", "Delete selected" buttons.
...
•	_edit_selected_med()
o	Gets med_id from selected row.
o	Finds row in schedule.
o	Calls _load_med_into_form.
o	Sets self.current_edit_med_id so _save_medication() knows to update, not insert.

•	_delete_selected_med()
o	Mark active = "0" in the schedule for that med_id.
o	Rewrites the CSV.
o	Refreshes table and grid.

•	_save_medication()
o	If self.current_edit_med_id is None → new med (compute ID via next_med_id() and append row).
o	Else → find row with that med_id, update fields, rewrite entire schedule via write_all().
...
•	_draw_summary()
o	Reads dose_log.csv, counts actions in last 7 days.
o	If matplotlib is available, draws a simple bar chart in self.summary_container.
o	Otherwise shows a “chart disabled” label.

D. Scheduler and popups
•	_scheduler_loop()
o	Called once at start, then re-schedules itself every 10 seconds.
o	Builds week using _build_week_schedule().
o	Filters to today’s doses only.
o	If any dose is within ±60 seconds of now and not yet logged (is_already_logged()), shows _show_due_popup(item) and breaks (only one popup per pass).
o	Always calls _update_grid_colors() afterward.

•	_show_due_popup(item)
o	Creates a modal Toplevel window with med name, dose, time.
o	Offers buttons:
	do_take() → logs "taken".
	do_snooze() → calls SnoozeManager.add() and logs "snoozed".
	do_skip() → logs "skipped".
o	Each handler updates the grid and shows a confirmation messagebox.

## 5. Known Issues
### 5.1 Minor issues (non-breaking)
•	Scheduler resolution
o	Checks every 10 seconds; if the app is paused or OS is busy, popups may be slightly delayed.
o	Time comparison window (±60 seconds) is a simple heuristic.

•	Fixed window size
o	self.geometry("1200x720") is hard-coded.
o	On very small screens, widgets may not fully fit.

•	CSV robustness
o	CSV parsing ignores malformed rows silently in several places (try/except with bare pass).
o	If someone manually edits the CSV incorrectly, it may cause subtle issues.

•	UI refresh logic
o	UI updates are sometimes triggered in multiple places (_scheduler_loop, _refresh_ui, after save/delete).
o	This is fine for this scale but could be tightened for clarity.

### 5.2 Major issues (breaking or potentially breaking)
•	Single-user assumption
o	No support for multiple users or profiles.
o	All meds go into one global schedule.

•	No persistent configuration
o	No config file for controlling behavior (e.g., grid buckets, snooze window).
o	All constants are in the code.

•	No unit tests
o	There is no automated test suite.
o	Changes must be tested manually by running the GUI.

### 5.3 Computational inefficiencies (optional note)
•	For the current scale (a few meds), the app is fine.
•	Inefficiencies:
o	_build_week_schedule() is called often and re-parses the CSV each time.
o	Logs and schedules are always read from disk instead of being cached.

•	For a realistic deployment with many entries, consider:
o	In-memory caching with explicit invalidation on change.
o	Using a lightweight database (e.g., SQLite) instead of CSV.

## 6. Future Work
Some directions for future development:
1.	Multi-user support
o	Add a concept of “profile” or “patient”.
o	Store a user_id column in CSV (or new DB schema).
o	Add UI to switch users.

2.	Time-travel / simulation mode
o	Allow developer to fake “current time” for testing.
o	Could be a hidden developer control in the Summary tab or a CLI flag.

3.	Editing log entries
o	Allow the user (or admin) to correct mistaken actions (e.g., clicked “skipped” instead of “taken”).

4.	Config file
o	Move constants (file paths, buckets, etc.) into a simple config module or JSON file.

5.	Improved error handling
o	Introduce a lightweight logging framework (logging module) instead of print().
o	Surface serious errors via a dedicated error dialog.

6.	Packaging / deployment
o	Wrap the app as a pip package or use PyInstaller to build a standalone executable.

## 7. Ongoing Development & Maintenance
If this project continues beyond the course, here are some suggestions:
•	Version control discipline
o	Keep run_app.py as the main entry point.
o	If the code grows, consider splitting into modules: model.py, view.py, controller.py, etc.
...
o	Use plain unittest or pytest.

•	Extensibility guidelines
o	New UI features should ideally:
	Reuse existing helpers for CSV I/O.
	Respect the current data model: med_schedule.csv, dose_log.csv, snoozes.csv.
o	When adding new fields, keep CSV headers and docstrings in sync.
